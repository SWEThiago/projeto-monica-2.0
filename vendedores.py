...
from dados import (
    adicionar_vendedor, remover_vendedor, obter_vendedor, 
    adicionar_lead, remover_lead, obter_leads, calcular_leads_por_status
)
...
def detalhes_vendedor(vendedor_id):
    vendedor = obter_vendedor(vendedor_id)
    if not vendedor:
        return redirect(url_for('vendedores.listar_vendedores'))

    leads = obter_leads(vendedor_id)
    stats = calcular_leads_por_status(vendedor_id)
...
def adicionar_lead_route(vendedor_id):
    ...
    adicionar_lead(vendedor_id, novo_lead)
...
def remover_lead_route(vendedor_id, lead_id):
    remover_lead(vendedor_id, lead_id)
    return redirect(url_for('vendedores.detalhes_vendedor', vendedor_id=vendedor_id))
